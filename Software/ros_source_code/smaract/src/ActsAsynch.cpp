...
bool ActsAsynch::Init(SA_INDEX *mcsHandle) {
    bool success_flag = true;
    const char loc[] = "network:192.168.1.200:5000";
    //const char loc[] = "usb:id:793748721"; //toggle to use USB
...
bool ActsAsynch::FindPosition_All(SA_INDEX mcsHandle, int *ConnChannels, double *pos_mm) {
    ...
    for(int i=0; i<6; i++) {
        if (ConnChannels[i] == 1) {
            result = SA_GetPosition_A(mcsHandle, i); //send get position of detected channels	            
			if(result != SA_OK) {
                COUT << "Failed to send GetPosition command for actuator #" << i << ". The status code is: " << result << "." << endl;
                success_flag = false;
            }
			    num_cmds_sent++; // incrementing up based on
        }
    }
    // Next, check packets from each channel
    for(int i=0; i<num_cmds_sent; i++) {
        do { 
            result = SA_ReceiveNextPacket_A(mcsHandle, 0, &packet); //wait zero seconds between            
			//COUT << "You have not received a packet of the right type yet" << endl;
				if (result != SA_OK) {
               	 	COUT << "Failed to send ReceiveNextPacket command. The status code is: " << result << "." << endl;
                	success_flag = false;
            	}
...
bool ActsAsynch::SetMaxFrequencies(SA_INDEX mcsHandle, int *ConnChannels, unsigned int max_freq) {
	...
	else COUT << "Max Frequency set" << endl;
    ...
    // Flush output and update buffer to put at unbuffered_output again
    SA_FlushOutput_A(mcsHandle);
    SA_SetBufferedOutput_A(mcsHandle, SA_UNBUFFERED_OUTPUT);
